...
% methods without MERRA2-GMI or UKML or NJML - for 2005 - 2022                        
...
'13000313-0A', ... 
'13000313-42', ...
'13000313-0E',}; % 5 deg.; for 2005 - 2022

% all_methods = {'10000133_go0' , ...
% '10000133_go3', ...
% '13000313-02'}; % 5 deg. or 20 deg., or 1991-2004
...
allYears = 2005:2022; % use [] for all years
...
picture_dpi = 600; % 300 or 600

metrics = {'R2', 'RMSE'}; % {'R2', 'RMSE', 'MAE', 'NMB'}

boxSize = 20.0;  % only important in phase-3 plots. % use [] for all box sizes

% Soft-data leakage control: set leakControl=1 (and matching leakRadius) to view
% the leakage-controlled CBV runs. Must match what was passed to runCBV.
leakControl = 0;       % 0=legacy files, 1=leakage-controlled (_lc) files
leakRadius  = 2.0;     % scalar deg or per-source struct (must match the run)
...
for eachYear = allYears
    fprintf('\n--- Processing Year: %d ---\n', eachYear);
    for i = 1:length(all_methods)
        configPattern = sprintf('CBV_BME%s*_box%d*_%d%s.mat', all_methods{i}, boxSize, eachYear, leakTag);
...
for i = 1:length(all_methods)
    configPatterns{i} = sprintf('CBV_BME%s*_box%d*%s.mat', all_methods{i}, boxSize, leakTag);
end
...
if ismember('phase3', plotLevels)
    ...
    figPaths = plotCBVresults_Phase3(repmat({cbvResultsDir}, 1, length(configPatterns)), configNames, ...
        ...
        'boxSize', boxSize, ...
        'metrics', metrics, ...
        'saveDir', fullfile(cbvResultsDir, 'figs', sprintf('box%g%s_%d_%d', boxSize, leakTag, allYears(1), allYears(end))), ...
        'dpi', picture_dpi, ...
        'leakTag', leakTag, ...
        'visible', 'on', ...
        'saveTables', true);
...
if ismember('phase4', plotLevels)
    ...
    figPaths_phase4 = plotCBVresults_Phase4(...
        ...
        'boxSize', boxSize, ...
        'metrics', metrics, ...
        'saveDir', fullfile(cbvResultsDir, 'figs', sprintf('phase4_box%g%s_%d_%d', boxSize, leakTag, allYears(1), allYears(end))), ...
        'dpi', picture_dpi, ...
        'leakTag', leakTag, ...
        'visible', 'on', ...
        'saveTables', true);
    ...
    fprintf('Phase 4 complete. Generated figures saved to: %s\n', ...
        fullfile(cbvResultsDir, 'figs', sprintf('phase4_box%g%s_%d_%d', boxSize, leakTag, allYears(1), allYears(end))));
end