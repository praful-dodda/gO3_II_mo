...
% SYNTAX :
%
% [zk,vk]=krigingME_stug(ck,ch,cs,zh,zs,vs,covmodel,covparam,nhmax,nsmax,dmax,order,options);
%
% INPUT :
%
...
for i=1:nk
  ...
  %   k0=coord2K(ck0,ck0,covmodel,covparam);             % compute the variance at ck0
  % else
  %   % k0=coord2K(ck0,ck0,covmodel,covparam)+vslocal(index(2));   % removed by MLS on Nov 7, 2013
  %   k0=coord2K(ck0,ck0,covmodel,covparam);             % Added by MLS on Nov 7, 2013   
  % end;
