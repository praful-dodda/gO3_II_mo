function [momentsXval1,momentsXval2,MSE,MAE,ME]=krigingME_Xvalidation(ckOption,ch,cs,zh,xs,vs,covmodel,covparam,nhmax,nsmax,dmax,order,options);
% BMEprobaMomentsXvalidation   - Cross validation for krigingME
%
% This program performs a cross-validation using krigingME.
...
elseif ~isempty(cs)
  ...
  error('ch and cs cannot both be empty');
end

%%%%%% Error messages

%[ch,zh,cs,nl,limi,probdens]=fixempty(ch,ch,zh,cs,nl,limi,probdens);
%[ch,zh,cs,nl,limi,probdens]=fixempty(ckDummy,ch,zh,cs,nl,limi,probdens);
...
for ik=1:nk                 % Number of estimation pts
  if ik <= nh
    chX=ch;
    if noindex==1
      chX(ik,:)=[];  
   ...
   %   limi,probdens,ik-nh);
      [csX,xsX,vsX]=probasplitME(cs,xs,vs,ik-nh);
  end
  if noindex==1
    ckX=ck(ik,:);
  else  
...
%      covmodel,covparam,nhmax,nsmax,dmax,order,options);
 [momentXval_1(ik),momentXval_2(ik)]=krigingME(ckX,chX,csX,zhX,xs,vs,covmodel,covparam,nhmax,nsmax,dmax,order);
...
MSE=mean((momentXval_1(:)-zdata).^2);
MAE=mean(abs(momentXval_1(:)-zdata));
ME=mean(momentXval_1(:)-zdata);
momentsXval1=momentXval_1;
momentsXval2=momentXval_2;