...
%% Input Validation
if nargin < 7
    error('All 7 input arguments required. See help estTOARsBMEoptim');
end
...
if ~exist(BMEsDir, 'dir')
    ...
    fprintf(fid, 'Generated by estTOARsBMEoptim.m\n');
...
if ~strcmpi(BMEparam.dataFormat, 'stug')
    error('BMEprobaType 3 (KrigingME) only implemented for dataFormat ''stug'' in this function.');
end
...
if iscell(KS.softdata)
    ...
    for ii = 1:length(KS.softdata)
        ...
        % vs must be the reformat cube's full-length (Zvar(:)) vector so it aligns with the
        % full-cube sub2ind index from neighbours_stug_optimized; KS.softdata{ii}.vs is compact
        % (valid-only) and overflows on any NaN-containing soft model (e.g. IASI-GOME2 with no
        % 2021). The single-dataset branch below already uses soft_data.vs.
...
for iTime = 1:length(tkVec)
    ...
    switch BMEprobaType
        case 1  % BMEprobaMoments
            % error
            error('BMEprobaType 1 (BMEprobaMoments) not implemented in this function.');
            
        case 2  % KrigingME
            % error
            error('BMEprobaType 2 (KrigingME) not implemented in this function.');
