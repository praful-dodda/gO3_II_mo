...
> **Provenance:** the `lambda1`/`lambda2` parquet files consumed here are generated by the
> **Python f-RAMP implementation in `f-RAMP-code/`, run on the UNC Longleaf cluster via
> SLURM** (`ramp_analysis_parallel.py` → `ramp_correction_parallel_v3.py`). MATLAB only
> *reads* them. The MATLAB `getOverhangRAMPv6_updated.m` at the repo root is a historical
> reference implementation and is **not** what produced these files, nor is it equivalent
> to the Python version. All sources — CTMs *and* satellite products — go through the same
> f-RAMP. See `f-RAMP-code/CLAUDE.md` for the algorithm, parameters, current-vs-legacy file
> map, and the soft-data leakage implications for CBV/LOOCV.
...
For questions about:
- **RAMP correction:** See `f-RAMP-code/CLAUDE.md` (Python f-RAMP, cluster-run)
